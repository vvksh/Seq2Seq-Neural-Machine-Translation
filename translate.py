...
def train():
  ...
  #from train is the index equivalent of 
  from_train = None
  to_train = None
  from_dev = None
  to_dev = None
  ...
  with tf.Session(config = config) as sess:
    # Create model.
    print("print from dev, to dev")
    print(from_dev, to_dev)
    print("print from dev, to dev")
    print(from_train, to_train)
...
def calculateBleu():
  ...
  #       outputs = []
  #       sorted_output_logits = [np.argsort(logit[0])[::-1] for logit in output_logits]
  #       # for logit in sorted_output_logits:
  #       #   print(logit)
  #       rank_logits = [0]*len(output_logits)

  #       for i in xrange(len(output_logits)):
  #         # output = []
  #         temp = sorted_output_logits[i][rank_logits[i]]
  #         # print (temp)
  #         if temp == data_utils.UNK_ID and rank_logits[i] < len(sorted_output_logits[0]) :
  #           rank_logits[i] = rank_logits[i]+1
  #           temp = sorted_output_logits[i][rank_logits[i]]
  #         if temp == data_utils.EOS_ID:
  #           break
  #         outputs.append(temp)

  #         # print( "output is",int(np.argmax(logit, axis=1)))
  #         # print()
